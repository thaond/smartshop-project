 ...
 * 		.registerTypeAdapter(Id.class, new IdTypeAdapter()).serializeNulls()
 * 		.setDateFormat(DateFormat.LONG).setFieldNamingPolicy(
 * 				FieldNamingPolicy.UPPER_CAMEL_CASE).setPrettyPrinting()
...
public final class GsonBuilder {
	...
	private static final ExcludeAnnotationSerializationExclusionStrategy excludeAnnotationSerializationExclusionStrategy = new ExcludeAnnotationSerializationExclusionStrategy();
	private static final ExcludeAnnotationDeserializationExclusionStrategy excludeAnnotationDeserializationExclusionStrategy = new ExcludeAnnotationDeserializationExclusionStrategy();
	...
	private boolean excludeFieldsWithExcludeAnnotaion;
	...
	public GsonBuilder() {
		...
		excludeFieldsWithExcludeAnnotaion = false;
	 ...
	 *         "Builder" pattern
	 */
	public GsonBuilder excludeFieldsWithExcludeAnnotation() {
		excludeFieldsWithExcludeAnnotaion = true;
		return this;
	}
	 ...
	 * applied as a disjunction rule. This means that if one of the {@code
	 * strategies} suggests that a field (or class) should be skipped then that
	 * field (or object) is skipped during serializaiton/deserialization.
	 * 
	 * @param strategies
	 ...
	 * 
	 * <p>
	 * Note that this pattern must abide by the convention provided by {@code
	 * SimpleDateFormat} class. See the documentation in
	...
	private <T> GsonBuilder registerDeserializer(Type typeOfT,
		...
		deserializers.register(typeOfT,
	 ...
	 * {@link InstanceCreator}, {@link JsonSerializer}, and a
	 * {@link JsonDeserializer}. It is best used when a single object {@code
	 * typeAdapter} implements all the required interfaces for custom
	...
	private <T> GsonBuilder registerSerializerForTypeHierarchy(
			Class<?> classOfT, final JsonSerializer<T> serializer) {
	...
	public Gson create() {
		...
		if (ignoreVersionsAfter != VersionConstants.IGNORE_VERSIONS) {
			serializationStrategies.add(new VersionExclusionStrategy(
					ignoreVersionsAfter));
			deserializationStrategies.add(new VersionExclusionStrategy(
					ignoreVersionsAfter));
		}
		...
		if (excludeFieldsWithExcludeAnnotaion) {
			serializationStrategies
					.add(excludeAnnotationSerializationExclusionStrategy);
			deserializationStrategies
					.add(excludeAnnotationDeserializationExclusionStrategy);
		}

		ExclusionStrategy serializationExclusionStrategy = new DisjunctionExclusionStrategy(
				serializationStrategies);
		ExclusionStrategy deserializationExclusionStrategy = new DisjunctionExclusionStrategy(
				deserializationStrategies);

		ParameterizedTypeHandlerMap<JsonSerializer<?>> customSerializers = serializers
				.copyOf();
		ParameterizedTypeHandlerMap<JsonDeserializer<?>> customDeserializers = deserializers
				.copyOf();
		addTypeAdaptersForDate(datePattern, dateStyle, timeStyle,
				customSerializers, customDeserializers);

		customSerializers.registerIfAbsent(DefaultTypeAdapters
				.getDefaultSerializers(serializeSpecialFloatingPointValues,
		...
		JsonFormatter formatter = prettyPrinting ? new JsonPrintFormatter(
				escapeHtmlChars) : new JsonCompactFormatter(escapeHtmlChars);
		Gson gson = new Gson(serializationExclusionStrategy,
				deserializationExclusionStrategy, fieldNamingPolicy,
				objConstructor, formatter, serializeNulls, customSerializers,
