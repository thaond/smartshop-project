...
var Lightbox = {	
	...
	slideShowTimer : null,
	startImage : null,
	beforefinish : null, //callbacks
	onfinish : null, //callbacks
	...
	initialize: function(options) {
		...
		this.options = $H({
			...
			totalImages : 0, //added for GWT - for caption display when images are loaded in batch
...
//		}
	...
	// Added for gwt integration
	gwtShowImage: function(imageUrl, imageTitle){
		this.gwtInitSlideShow();
		this.imageArray.push({'link':imageUrl, 'title':imageTitle});
		this.startImage = 0;
		this.gwtStart();
	},
	
	gwtCreateNewSlideShow: function(startImageNo, setName, imageUrls, imageTitles){
		this.gwtInitSlideShow();
		...
		this.startImage = startImageNo;

		// get group name
		this.groupName = setName;
		this.gwtStart();
	},

	gwtAddImagesToCurrentSlideShow: function(imageUrls, imageTitles){
		for (var i=0; i<imageUrls.length; i++){
			var imageUrl = imageUrls[i];
			var imageTitle = imageTitles[i];
			this.imageArray.push({'link':imageUrl,'title':imageTitle});
	...
	// start function added for gwt integration	
	gwtStart: function(){
	...
	//function to initialize slideshow, added for gwt integration
	gwtInitSlideShow: function(){
		this.hideBadObjects();
		this.imageArray = [];
		this.groupName = null;
	},
	
	gwtSetOptions: function(option){
		this.options.merge(option);
	},

	gwtGetOptions: function(option,name){
		option[name] = this.options[name];
	},

	//function added to change values after slideshow initialization
	gwtSetResizeSpeed: function(speed){
		if(this.options.animate){
			this.options.resizeSpeed = Math.max(Math.min(this.options.resizeSpeed,10),1);
			this.resizeDuration = (11 - this.options.resizeSpeed) * 0.15;
		}else{
	...
	//function added to change values after slideshow initialization	
	gwtSetOverlayDuration: function(duration){
		if(this.options.animate){
			this.overlayDuration = Math.max(this.options.overlayDuration,0);
		}else{
			this.overlayDuration = 0;
	...
	gwtGetLocaleOptions: function(name){
		return this.options.strings[name];
	},

	gwtSetAnimate: function(animate){
	...
	gwtGetTotalImages: function(){
			if(this.options.totalImages == 0)
			return this.imageArray.length;
		else
			return this.options.totalImages;
	},
	...
	updateDetails: function() {
		...
		if(this.imageArray.length > 1){
			...
			var num_display = this.options.strings.numDisplayPrefix + ' ' + eval(this.activeImage + 1) + ' ' + this.options.strings.numDisplaySeparator + ' ' + Lightbox.gwtGetTotalImages();
	...
	showNext : function(){
		if(this.imageArray.length > 1){
			if(!this.options.loop && ((this.activeImage == this.imageArray.length - 1 && this.startImage == 0) || (this.activeImage+1 == this.startImage))){
				if(this.onfinish){
					this.onfinish.apply(self);
				}
				return this.end();
			}
			if(this.beforefinish && this.activeImage == this.imageArray.length - 2){
				this.beforefinish.apply();
			}
