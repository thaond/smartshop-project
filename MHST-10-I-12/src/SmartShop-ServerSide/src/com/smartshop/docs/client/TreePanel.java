...
public class TreePanel extends VerticalPanel {
	...
	private TreePanel() {
		...
		TreeItem treeGuide = new TreeItem("Hướng dẫn sử dụng");
		ItemNode itemInstall = new ItemNode("Download", "android-install",
				"/doc/1_Cai_Dat_Ung_Dung.htm");
		
		treeGuide.addItem(itemInstall);

		rootAndroid.addItem(itemIntroAnd);
		rootAndroid.addItem(itemDownload);
		rootAndroid.addItem(treeGuide);
